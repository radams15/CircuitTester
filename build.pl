...
if($ARGV[0] eq "build"){
	...
	print `podman run -u=builder:builder --volume /mnt/files/scripts/CircuitTester:/CircuitTester:z -ti qtlin6 /bin/build`;
...
}elsif($ARGV[0] eq "run5"){
	print `podman run --volume /mnt/files/scripts/CircuitTester:/CircuitTester:z -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$DISPLAY -ti qtlin /CircuitTester/linbuild5/src/main/CircuitTester`;
}elsif($ARGV[0] eq "run6"){
	print `podman run --volume /mnt/files/scripts/CircuitTester:/CircuitTester:z -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$DISPLAY -ti qtlin6 /CircuitTester/linbuild6/src/main/CircuitTester`;
}else{
