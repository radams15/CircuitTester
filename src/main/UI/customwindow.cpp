...
CustomWindow::CustomWindow(QWidget *parent) : QMainWindow(parent), RESIZE_LIMIT(4){
    ...
    QFile toolbarIn(":/css/custom_toolbar.css");
    toolbarIn.open(QFile::ReadOnly);
    custom_toolbar_css = toolbarIn.readAll().toStdString();

    QFile windowIn(":/css/custom_window.css");
    windowIn.open(QFile::ReadOnly);
    custom_window_css = windowIn.readAll().toStdString();

    setStyleSheet(QString::fromStdString(custom_window_css));
...
QToolBar *CustomWindow::generateBorder(Qt::ToolBarArea area,
    ...
    border->setStyleSheet(QString::fromStdString(custom_toolbar_css));
