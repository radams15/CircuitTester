...
TEST(MNACircuit, CorrectCategorisation){
    auto b1 = Component(0, 1, MNA_BATTERY, 2);
    auto b2 = Component(1, 2, MNA_BATTERY, 4);
    auto r1 = Component(2, 0, MNA_RESISTOR, 1);

    auto c = Circuit({b1, r1, b2});
...
TEST(MNACircuit, CorrectRefNodes2){
    ...
    auto b2 = Component(3, 4, MNA_BATTERY, 4);
    auto b3 = Component(4, 3, MNA_BATTERY, 4);

    auto c = Circuit({b1, r1, b2, b3});
...
TEST(MNACircuit, GetCurrentsInvalidSign){
    auto c = Circuit({Component(0, 1, MNA_BATTERY, 2)});
